...
function initPage() {
  ...
  // On commence par differencier de l'orderId qui servira pour la confirmation

  bindIt();
  const cart = getCart();
  // ici let cart ca ou ca
...
// const selectForm = document.form[0];
...
async function loadDetailProd(cart) {
  ...
  for (let i in cart) {
    ...
    //ici await fetch(urlApi + "/" + idProduct);
  ...
  detailProd.map((prod) => {
    totPrice += globalPrice(prod);
    totQuant += parseInt(prod.quantities);

    cartItems.append(displayCart(prod));
  });

  if (!updateTotalQty(totQuant) || !updateTotalCart(totPrice)) {
    while (cartItems.hasChildNodes()) {
      cartItems.removeChild(cartItems.firstChild);
    }
    alert("An error occured while loading the cart!");
  }
}

function displayCart(prod) {
  const article = makeElem("article", "class", "cart_item");
